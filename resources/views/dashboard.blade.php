...
<head>
    ...
    <style>
        ...
        #search-container { transition: all 0.7s cubic-bezier(0.25, 1, 0.5, 1); }
        #welcome-section { transition: all 0.5s ease-out; }
        .fade-in-up { opacity: 0; transform: translateY(20px); transition: all 0.6s cubic-bezier(0.25, 1, 0.5, 1); }
        ...
        .save-spinner { display: none; width: 16px; height: 16px; border: 2px solid rgba(255,255,255,0.3); border-radius: 50%; border-top-color: #fff; animation: spin 0.8s linear infinite; }
        @keyframes spin { to { transform: rotate(360deg); } }
        .save-btn.loading .save-spinner { display: inline-block; }
        .save-btn.loading .save-text { display: none; }
        .save-btn.saved { background-color: #10b981 !important; border-color: #10b981 !important; color: white !important; }
        .save-btn.saved .save-text { display: inline; }
        .save-btn.saved .save-spinner { display: none; }
    </style>
</head>
...
<body class="font-inter flex min-h-screen overflow-x-hidden selection:bg-blue-100 selection:text-blue-900">
    ...
    <aside id="sidebar" class="fixed inset-y-0 left-0 w-72 bg-white border-r border-slate-100 z-50 flex flex-col transform -translate-x-full lg:translate-x-0 transition-transform duration-300 shadow-[4px_0_24px_rgba(0,0,0,0.02)]">
        ...
        <nav class="flex-1 overflow-y-auto py-6 px-4 space-y-1">
            ...
            <a href="{{ route('dashboard') }}" class="flex items-center gap-3 px-4 py-2.5 rounded-xl bg-blue-50 text-blue-700 font-medium transition-colors">
        ...
        <div class="p-4 border-t border-slate-100">
            <div class="flex items-center gap-3 p-3 rounded-2xl hover:bg-slate-50 transition-colors cursor-pointer group">
                <div class="w-10 h-10 rounded-full bg-blue-100 text-blue-700 flex items-center justify-center font-bold">{{ substr($userName, 0, 1) }}</div>
                ...
                <!-- ====== FIXED LOGOUT FORM ====== -->
                <form action="{{ route('logout') }}" method="POST" class="ml-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    @csrf
    ...
    <main class="flex-1 lg:ml-72 flex flex-col relative w-full h-screen overflow-y-auto">
        ...
        <div id="top-spacer" class="h-[25vh] transition-all duration-700 ease-in-out shrink-0 w-full hidden md:block"></div>

        <div id="search-container" class="w-full max-w-4xl mx-auto px-4 sm:px-8 py-8 md:py-0 flex flex-col sticky top-0 z-20">
            <div id="welcome-section" class="text-center mb-8 overflow-hidden">
                <h1 class="font-poppins text-4xl sm:text-5xl font-bold text-slate-900 mb-4 tracking-tight">Welcome back, {{ $userName }}</h1>
            ...
            <form id="search-form" onsubmit="executeSearch(event)" class="relative w-full group">
                ...
                <input type="text" id="search-input" autocomplete="off" placeholder="Search papers, authors, DOI, journals..."
                ...
                <button type="submit" class="absolute inset-y-2 right-2 px-6 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-2xl transition-colors shadow-sm flex items-center gap-2">Search <span class="hidden sm:inline">→</span></button>
            </form>

            <div id="suggestion-chips" class="mt-8 flex flex-wrap justify-center gap-3 transition-opacity duration-300">
                @php $chips = ['Artificial Intelligence', 'Machine Learning', 'Cybersecurity', 'Healthcare', 'Software Engineering', 'Blockchain']; @endphp
                @foreach($chips as $chip)
                    <button onclick="setSearch('{{ $chip }}')" class="px-5 py-2.5 rounded-full border border-slate-200 bg-white text-slate-600 text-sm font-medium hover:border-blue-300 hover:bg-blue-50 hover:text-blue-700 transition-all shadow-sm">{{ $chip }}</button>
                @endforeach
            </div>
        </div>
        ...
        <div id="main-content" class="hidden w-full max-w-5xl mx-auto px-4 sm:px-8 pb-20 flex-1">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 bg-white p-4 rounded-2xl border border-slate-200 shadow-[0_2px_10px_rgb(0,0,0,0.02)]">
                <div class="flex flex-wrap items-center gap-3">
                    ...
                    <select id="year-filter" class="bg-slate-50 border border-slate-200 text-sm font-medium text-slate-700 rounded-xl px-4 py-2 hover:bg-slate-100 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all cursor-pointer">
                    ...
                    <select id="oa-filter" class="bg-slate-50 border border-slate-200 text-sm font-medium text-slate-700 rounded-xl px-4 py-2 hover:bg-slate-100 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all cursor-pointer">
                        <option value="">Open Access</option>
                        <option value="open">Only Open Access</option>
                    </select>
                    <select id="source-filter" class="bg-slate-50 border border-slate-200 text-sm font-medium text-slate-700 rounded-xl px-4 py-2 hover:bg-slate-100 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all cursor-pointer">
                ...
                <div class="flex items-center gap-3 pt-4 md:pt-0 border-t md:border-t-0 md:border-l border-slate-100 md:pl-4">
                    ...
                    <select id="sort-filter" class="bg-blue-50 border border-blue-100 text-blue-700 text-sm font-semibold rounded-xl px-4 py-2 hover:bg-blue-100 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/30 transition-all cursor-pointer">
            ...
            <div id="results-feed" class="space-y-5 fade-in-up"></div>

            <div id="load-more-container" class="hidden mt-8 text-center fade-in-up">
                <button id="load-more-btn" onclick="loadMore()" class="px-8 py-3 bg-white border border-slate-200 hover:border-blue-300 hover:bg-blue-50 text-slate-700 hover:text-blue-700 font-medium rounded-2xl shadow-sm transition-all duration-200 inline-flex items-center gap-2">
            ...
            <div id="post-search-sections" class="fade-in-up hidden mt-16 space-y-12 border-t border-slate-200 pt-12">
                <div>
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <button class="flex flex-col items-center justify-center p-6 bg-white border border-slate-200 rounded-2xl hover:border-blue-400 hover:shadow-md transition-all group">
                        ...
                        <button class="flex flex-col items-center justify-center p-6 bg-white border border-slate-200 rounded-2xl hover:border-blue-400 hover:shadow-md transition-all group">
                            <div class="w-12 h-12 bg-indigo-50 text-indigo-600 rounded-xl flex items-center justify-center mb-3 group-hover:scale-110 transition-transform">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div>
                        <h3 class="font-poppins text-xl font-semibold text-slate-900 mb-6 flex items-center gap-2"><span class="text-purple-500">✨</span> AI Recommendations</h3>
                        <div class="space-y-4">
                            <div class="p-5 bg-white border border-slate-100 rounded-2xl shadow-sm hover:shadow-md transition-all cursor-pointer">
                                <h4 class="font-semibold text-slate-900 line-clamp-2 mb-1">Transformers in Healthcare: A systematic review of medical language models</h4>
                                <p class="text-sm text-slate-500">Chen et al. • 2023</p>
                            </div>
                            <div class="p-5 bg-white border border-slate-100 rounded-2xl shadow-sm hover:shadow-md transition-all cursor-pointer">
                                <h4 class="font-semibold text-slate-900 line-clamp-2 mb-1">Zero-shot learning capabilities in modern architectures</h4>
                                <p class="text-sm text-slate-500">Smith, J. • 2024</p>
                            </div>
                        </div>
                    </div>
                    <div>
                        <h3 class="font-poppins text-xl font-semibold text-slate-900 mb-6 flex items-center gap-2"><span class="text-orange-500">🔥</span> Trending Papers</h3>
                        <div class="space-y-4">
                            <div class="p-5 bg-white border border-slate-100 rounded-2xl shadow-sm hover:shadow-md transition-all cursor-pointer">
                                <h4 class="font-semibold text-slate-900 line-clamp-2 mb-1">Attention Is All You Need</h4>
                                <p class="text-sm text-slate-500 mb-2">Vaswani et al. • NIPS • 2017</p>
                                <div class="inline-flex items-center gap-1 text-xs font-medium text-emerald-600 bg-emerald-50 px-2 py-1 rounded-md">
    ...
    <!-- MODALS -->
    <div id="ai-modal" class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[100] hidden flex items-center justify-center p-4 transition-opacity opacity-0">
        <div class="bg-white rounded-3xl w-full max-w-2xl max-h-[85vh] overflow-hidden flex flex-col shadow-2xl transform scale-95 transition-transform duration-300 modal-content">
            <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-white z-10">
                <h3 class="font-poppins text-lg font-semibold text-slate-900 flex items-center gap-2"><span class="text-purple-500">✨</span> AI Paper Summary</h3>
                <button onclick="closeModal('ai-modal')" class="p-2 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded-xl transition-colors">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                </button>
            </div>
            <div class="p-6 overflow-y-auto space-y-6">
                <div><h4 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Summary</h4><p class="text-slate-700 leading-relaxed text-sm">This paper introduces a novel approach to optimizing large language models for resource-constrained environments. By leveraging sparse attention mechanisms, the authors achieve comparable accuracy while reducing compute requirements by 40%.</p></div>
                <div><h4 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Key Findings</h4><ul class="list-disc list-inside text-slate-700 text-sm space-y-1"><li>40% reduction in computational overhead.</li><li>Maintained 98% baseline accuracy on standard benchmarks.</li><li>Effective scaling laws demonstrated up to 70B parameters.</li></ul></div>
                <div><h4 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Methodology</h4><p class="text-slate-700 leading-relaxed text-sm">The authors utilized a custom sparse attention kernel combined with structured pruning techniques during the fine-tuning phase.</p></div>
                <div><h4 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Conclusion & Future Work</h4><p class="text-slate-700 leading-relaxed text-sm">The architecture presents a viable path for deploying advanced AI on edge devices. Future work will explore dynamic sparsity patterns based on input context.</p></div>
            </div>
            <div class="px-6 py-4 border-t border-slate-100 bg-slate-50 flex justify-end gap-3">
                <button onclick="closeModal('ai-modal')" class="px-5 py-2.5 text-slate-600 font-medium hover:bg-slate-200 rounded-xl transition-colors">Close</button>
                <button class="px-5 py-2.5 bg-purple-600 hover:bg-purple-700 text-white font-medium rounded-xl shadow-sm transition-colors flex items-center gap-2">
        ...
        <div class="bg-white rounded-3xl w-full max-w-lg overflow-hidden flex flex-col shadow-2xl transform scale-95 transition-transform duration-300 modal-content">
            ...
            <div class="p-6">
                <div class="flex gap-2 mb-4 overflow-x-auto no-scrollbar pb-2">
                    <button class="px-4 py-1.5 rounded-full bg-blue-600 text-white text-sm font-medium whitespace-nowrap shadow-sm">APA</button>
                    <button class="px-4 py-1.5 rounded-full border border-slate-200 text-slate-600 hover:bg-slate-50 text-sm font-medium whitespace-nowrap">MLA</button>
                    <button class="px-4 py-1.5 rounded-full border border-slate-200 text-slate-600 hover:bg-slate-50 text-sm font-medium whitespace-nowrap">Chicago</button>
                    <button class="px-4 py-1.5 rounded-full border border-slate-200 text-slate-600 hover:bg-slate-50 text-sm font-medium whitespace-nowrap">IEEE</button>
                </div>
                <div class="bg-slate-50 border border-slate-200 p-4 rounded-2xl relative group">
                    <p class="text-slate-800 text-sm font-serif leading-relaxed" id="citation-text">Doe, J., Smith, A., & Johnson, B. (2024). A comprehensive analysis of language models. <i>Journal of Artificial Intelligence Research</i>, 45(2), 112-130. https://doi.org/10.1234/jair.2024.45.2.112</p>
                    <button onclick="copyCitation()" class="absolute top-2 right-2 p-2 bg-white border border-slate-200 rounded-lg text-slate-500 hover:text-blue-600 shadow-sm opacity-0 group-hover:opacity-100 transition-all">
            ...
            <div class="px-6 py-4 border-t border-slate-100 bg-slate-50 flex justify-end gap-3">
                ...
                <button onclick="copyCitation()" class="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-xl shadow-sm transition-colors flex items-center gap-2">Copy Citation</button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function saveSearchState(query, results, page) {
            sessionStorage.setItem('lastSearch', JSON.stringify({ query, results, page: page || 1 }));
        }
        ...
        async function fetchPapers(page, isAppend = false) {
            ...
            try {
                ...
                const params = new URLSearchParams({ q: currentQuery, page: page });
                ...
                if (data.success && data.results.length > 0) {
                    ...
                    if (page * data.per_page < data.total) showLoadMoreButton();
        ...
        // ============================================
        // RENDER RESULTS – with saved flag and data-doi
        // ============================================
        function renderResults(papers, isAppend = false) {
            ...
            papers.forEach((paper, index) => {
                ...
                const isSaved = paper.saved || false;

                let paperUrl;
                if (paper.doi) {
                    paperUrl = `/paper/${encodeURIComponent(paper.doi)}`;
                } else {
                    ...
                    paperUrl = `/paper?${params.toString()}`;
                }
                ...
                const cardHTML = `
                    <div class="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm hover:shadow-lg transition-all duration-300" style="${animationStyle}">
                        <div class="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-3">
                            <h2 class="font-poppins text-xl font-bold text-slate-900 leading-tight md:pr-12">${paper.title}</h2>
                            <div class="shrink-0">
                        ...
                        <div class="flex flex-wrap items-center gap-3 pt-4 border-t border-slate-50">
                            <a href="${paperUrl}" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-xl shadow-sm transition-colors">View Paper</a>
                            <button data-doi="${paper.doi || ''}" 
                                    data-paper='${JSON.stringify(paper).replace(/'/g, "&#39;")}'
                                    onclick="savePaper(this)" 
                                    class="save-btn px-4 py-2 ${isSaved ? 'saved' : 'bg-white border border-slate-200 hover:bg-slate-50 text-slate-700'} rounded-xl transition-colors flex items-center gap-2">
        ...
        // ============================================
        // SAVE PAPER – TOGGLE SAVE/UNSAVE with DOI
        // ============================================
        async function savePaper(button) {
            ...
            const paperDataAttr = button.getAttribute('data-paper');
            let paperData = null;
            if (paperDataAttr) {
                try { paperData = JSON.parse(paperDataAttr); } catch (e) {}
            }
            ...
            try {
                ...
                if (response.ok && result.success) {
                    // Update all buttons with the same DOI on the page
                    document.querySelectorAll(`button[data-doi="${doi}"]`).forEach(btn => {
                        btn.classList.remove('loading');
                        if (isCurrentlySaved) {
                            // Unsave
                            btn.classList.remove('saved');
                            btn.classList.add('bg-white', 'border', 'border-slate-200', 'hover:bg-slate-50', 'text-slate-700');
                            btn.querySelector('.save-text').textContent = 'Save';
                        } else {
                            // Save
                            btn.classList.add('saved');
                            btn.classList.remove('bg-white', 'border-slate-200', 'hover:bg-slate-50', 'text-slate-700');
                            btn.querySelector('.save-text').textContent = 'Saved';
                        }
                        btn.disabled = false;
                    });
                    showToast(isCurrentlySaved ? 'Removed' : 'Saved!', result.message, 'success');
                } else {
                    // Revert this button
        ...
        function triggerLayoutShift() {
            ...
            if (spacer) spacer.classList.replace('h-[25vh]', 'h-0');
        ...
        // ============================================
        // FILTER CHANGE HANDLERS & DOM READY
        // ============================================
        document.addEventListener('DOMContentLoaded', function () {
            ...
            filters.forEach(id => {
                ...
                if (el) {
                    el.addEventListener('change', function () {
                        if (currentQuery) executeSearch(new Event('submit'));
                    });
                }
            });

            const style = document.createElement('style');
            style.innerHTML = `@keyframes fadeSlideUp { from { opacity: 0; transform: translateY(20px); } to { opacity: 1; transform: translateY(0); } }`;
            ...
            if (savedState && savedState.query) {
                ...
                if (savedState.results && savedState.results.length > 0) {
                    ...
                    if (savedState.results.length >= 10) showLoadMoreButton();
