...
<body class="font-inter flex min-h-screen overflow-x-hidden selection:bg-blue-100 selection:text-blue-900">
    ...
    <aside id="sidebar" class="fixed inset-y-0 left-0 w-72 bg-white border-r border-slate-100 z-50 flex flex-col transform -translate-x-full lg:translate-x-0 transition-transform duration-300 shadow-[4px_0_24px_rgba(0,0,0,0.02)]">
        ...
        <nav class="flex-1 overflow-y-auto py-6 px-4 space-y-1">
            ...
            <a href="{{ route('assistant') }}" class="flex items-center gap-3 px-4 py-2.5 rounded-xl bg-blue-50 text-blue-700 font-medium transition-colors">
    ...
    <main class="flex-1 lg:ml-72 flex flex-col relative w-full h-screen overflow-hidden bg-[#F8FAFC]">
        ...
        <!-- Header -->
        <div class="flex items-center justify-between px-6 py-4 bg-white/90 backdrop-blur-md border-b border-slate-200/80 shrink-0 z-10">
            <div class="flex items-center gap-3">
            ...
            <div class="flex items-center gap-2">
                <button class="flex items-center gap-2 px-3.5 py-1.5 border border-slate-200 text-slate-600 hover:bg-slate-50 hover:text-slate-900 text-xs font-semibold rounded-xl transition-all shadow-sm">
        ...
        <div id="chat-stream" class="flex-1 overflow-y-auto px-4 sm:px-8 py-8 space-y-6">
            <div class="max-w-4xl mx-auto space-y-6">
                ...
                <div class="flex items-start gap-4">
                    <div class="w-10 h-10 rounded-xl bg-blue-600 text-white flex items-center justify-center shrink-0 shadow-md shadow-blue-600/10 mt-1">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path></svg>
                    </div>
                    <div class="flex-1 bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm space-y-4">
                        ...
                        <div class="pt-2 border-t border-slate-100">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                <button class="flex items-start gap-3 p-3 text-left rounded-xl border border-slate-200/80 hover:border-blue-300 hover:bg-blue-50/50 transition-all group">
                                    <div class="p-2 bg-slate-100 group-hover:bg-blue-100/60 rounded-lg text-slate-600 group-hover:text-blue-700 transition-colors">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-[11px] text-slate-500 mt-0.5">Synthesize key findings across saved library items</p>
                                    </div>
                                </button>
                ...
                <div class="flex items-start justify-end gap-3 pl-12">
                    <div class="bg-blue-600 text-white px-5 py-4 rounded-2xl rounded-tr-none shadow-sm shadow-blue-600/10 max-w-2xl">
                ...
                <div class="flex items-start gap-4 pr-12">
                    ...
                    <div class="flex-1 bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm space-y-4">
                        <p class="text-sm text-slate-700 leading-relaxed">
                            Standard attention scales quadratically $O(N^2)$ with input sequence length. <span class="font-semibold text-slate-900">Sparse attention mechanisms</span> reduce this overhead by selectively evaluating token dependencies rather than computing full attention matrices.
                        </p>
                        ...
                        <div class="grid grid-cols-1 gap-2.5">
                            <div class="p-3 bg-slate-50 border border-slate-100 rounded-xl">
                                ...
                                <span class="text-xs text-slate-600">Restricts attention computation strictly to neighboring tokens within a fixed radius $k$.</span>
                            </div>
                            <div class="p-3 bg-slate-50 border border-slate-100 rounded-xl">
                                <span class="text-xs font-semibold text-slate-900 block mb-0.5">2. Strided / Fixed Patterns</span>
                                <span class="text-xs text-slate-600">Evaluates attention at regular interval steps to gather global context across long sequences.</span>
                            </div>
                            <div class="p-3 bg-slate-50 border border-slate-100 rounded-xl">
                                <span class="text-xs font-semibold text-slate-900 block mb-0.5">3. Global Anchor Tokens</span>
                                <span class="text-xs text-slate-600">Designates specific tokens that interact with the entire sequence to propagate high-level context.</span>
                            </div>
                        </div>
        ...
        <div class="p-4 sm:p-6 bg-white border-t border-slate-200/80 shrink-0">
            <div class="max-w-4xl mx-auto space-y-3">
                <div class="relative flex items-center">
                    <button class="absolute left-4 p-1.5 text-slate-400 hover:text-blue-600 rounded-lg hover:bg-slate-100 transition-colors" title="Attach Literature File">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13"></path></svg>
                    </button>

                    <input type="text" placeholder="Ask anything about research papers, methodologies, or citations..." class="w-full py-4 pl-12 pr-28 text-sm bg-slate-50 border border-slate-200 rounded-2xl text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-600 focus:bg-white transition-all">

                    <button class="absolute right-2 px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold rounded-xl transition-colors shadow-sm shadow-blue-600/20 flex items-center gap-2">
                        <span>Send</span>
                ...
                <div class="flex items-center justify-between text-[11px] text-slate-400 px-1">
                    <span class="flex items-center gap-1.5">
                        ...
                        Press <kbd class="px-1.5 py-0.5 bg-slate-100 border border-slate-200 rounded text-slate-600 font-mono">Enter</kbd> to send
                    </span>
